...
void ASideScrollerCharacter::Tick(float DeltaTime)
{
	...
	float Power = (PowerTimer > 0) ? PowerMultiplier : 1.0f;

	GetCharacterMovement()->MaxWalkSpeed = WalkSpeed * Power;
...
void ASideScrollerCharacter::CalculateHealth(float Delta)
{
	Health += Delta;
...
void ASideScrollerCharacter::PickUp(float time)
{
	PowerTimer = time;
}

void ASideScrollerCharacter::CalculateDeath()
{
	IsDead = (Health <= 0.0f);
}
